package dev.braintrust.claude.test.otel;
...
/**
 * AssertJ assertions for OpenTelemetry spans. Provides fluent API for testing span attributes,
 * events, and timing.
 */
public class SpanAssertions extends AbstractAssert<SpanAssertions, SpanData> {
    ...
    public SpanAssertions hasStatusWithDescription(StatusCode code, String description) {
        ...
        if (!actual.getStatus().getDescription().equals(description)) {
            failWithMessage(
                    "Expected status description to be <%s> but was <%s>",
    ...
    public <T> SpanAssertions hasAttribute(AttributeKey<T> key, T value) {
        ...
        if (!value.equals(actualValue)) {
            failWithMessage(
                    "Expected attribute <%s> to be <%s> but was <%s>", key, value, actualValue);
        }
        return this;
    }
    ...
    public SpanAssertions hasAttributeCount(int count) {
        ...
        if (actualCount != count) {
            failWithMessage(
                    "Expected span to have <%d> attributes but had <%d>", count, actualCount);
        }
        return this;
    }
    ...
    public SpanAssertions hasEventCount(int count) {
        ...
        if (actualCount != count) {
            failWithMessage("Expected span to have <%d> events but had <%d>", count, actualCount);
        }
        return this;
    }
    ...
    public SpanAssertions hasEvent(String name) {
        isNotNull();
        boolean found = actual.getEvents().stream().anyMatch(event -> event.getName().equals(name));
        if (!found) {
    ...
    public SpanAssertions hasExceptionEventWithType(Class<? extends Throwable> exceptionType) {
        ...
        if (!found) {
            failWithMessage(
                    "Expected span to have exception event of type <%s>", exceptionType.getName());
        }
        return this;
    }
    ...
    public SpanAssertions hasDurationBetween(Duration min, Duration max) {
        ...
        if (duration.compareTo(min) < 0 || duration.compareTo(max) > 0) {
            failWithMessage(
                    "Expected span duration to be between <%s> and <%s> but was <%s>",
                    min, max, duration);
        }
        return this;
    }
    ...
    public SpanAssertions startedAfter(Instant time) {
        ...
        if (!startTime.isAfter(time)) {
            failWithMessage(
                    "Expected span to start after <%s> but started at <%s>", time, startTime);
        }
        return this;
    }
    ...
    public SpanAssertions endedBefore(Instant time) {
        ...
        if (!endTime.isBefore(time)) {
            failWithMessage("Expected span to end before <%s> but ended at <%s>", time, endTime);
        }
        return this;
    }
    ...
    public SpanAssertions hasParentSpanId(String parentSpanId) {
        ...
        if (!actual.getParentSpanId().equals(parentSpanId)) {
            failWithMessage(
                    "Expected parent span ID to be <%s> but was <%s>",
    ...
    public SpanAssertions hasNoParent() {
        ...
        if (!actual.getParentSpanId().isEmpty()) {
            failWithMessage(
                    "Expected span to have no parent but had parent ID <%s>",
