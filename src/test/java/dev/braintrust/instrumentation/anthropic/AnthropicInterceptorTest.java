...
class AnthropicInterceptorTest {
    ...
    void testTraceMessageSuccess() {
        ...
        // When
        var result = AnthropicInterceptor.traceMessage(
            request,
            req -> new AnthropicInterceptor.RequestDetails(req.model, req.maxTokens, req.temperature),
            resp -> new AnthropicInterceptor.ResponseDetails(
        ...
        assertThat(attributes.get(io.opentelemetry.api.common.AttributeKey.stringKey("anthropic.model")))
            .isEqualTo("claude-3-opus");
        assertThat(attributes.get(io.opentelemetry.api.common.AttributeKey.longKey("anthropic.max_tokens")))
            .isEqualTo(2000L);
        assertThat(attributes.get(io.opentelemetry.api.common.AttributeKey.doubleKey("anthropic.temperature")))
            .isEqualTo(0.8);
    ...
    void testTraceMessageAsync() throws Exception {
        ...
        // When
        var future = AnthropicInterceptor.traceMessageAsync(
            request,
            req -> new AnthropicInterceptor.RequestDetails(req.model, req.maxTokens, req.temperature),
            resp -> new AnthropicInterceptor.ResponseDetails(
                new AnthropicInterceptor.Usage(
        ...
        assertThat(span.getAttributes().get(BraintrustSpanProcessor.USAGE_COMPLETION_TOKENS)).isEqualTo(150L);
    }
    ...
    void testTraceStreamingMessage() {
        ...
        // When
        var result = AnthropicInterceptor.traceStreamingMessage(
            request,
            req -> new AnthropicInterceptor.RequestDetails(req.model, req.maxTokens, req.temperature),
            () -> stream,
        ...
        assertThat(span.getAttributes().get(io.opentelemetry.api.common.AttributeKey.booleanKey("anthropic.stream")))
            .isTrue();
    ...
    void testCostCalculation() {
        ...
        for (var testCase : testCases) {
            ...
            assertThat(cost)
                .as("Cost for model %s with %d input and %d output tokens", 
    ...
    void testConcurrentRequests() throws Exception {
        ...
        for (int i = 0; i < requestCount; i++) {
            ...
            futures[i] = CompletableFuture.supplyAsync(() -> {
                ...
                var response = new TestResponse(new TestUsage(100 + index, 50 + index));
        ...
        var tokenCounts = spans.stream()
            .map(span -> span.getAttributes().get(BraintrustSpanProcessor.USAGE_PROMPT_TOKENS))
            .toList();
