...
class ScorerTest {
    ...
    void testCustomScorer() {
        // Given
        Scorer<String, Integer> lengthScorer = new Scorer<>() {
            @Override
    ...
    void testScorerFactoryMethod() {
        // Given
        var scorer = Scorer.of("custom", (String input, String output) -> 
    ...
    void testRegexScorer() {
        // Given
        var scorer = Scorer.StringScorers.regex(
            (String input) -> Pattern.compile("\\d{3}-\\d{4}")
        );
        
        // When/Then
        assertThat(scorer.score(null, "123-4567")).isEqualTo(1.0);
        assertThat(scorer.score(null, "Call me at 555-1234")).isEqualTo(0.0); // Must match entire string
    ...
    void testAbsoluteErrorScorer() {
        // Given
        var scorer = Scorer.NumericScorers.absoluteError(
            (String input) -> 100.0,
            10.0 // threshold
        );
        ...
        assertThat(scorer.score(null, 120.0)).isCloseTo(0.8, within(0.01)); // Normalized by expected
    ...
    void testRelativeErrorScorer() {
        // Given
        var scorer = Scorer.NumericScorers.relativeError(
            (String input) -> 100.0,
            0.1 // max 10% error
        );
        ...
        var zeroScorer = Scorer.NumericScorers.relativeError(
            (String input) -> 0.0,
            0.1
        );
    ...
    void testCombinedScorer() {
        ...
        var avgScorer = Scorer.combine(
            "average",
            List.of(scorer1, scorer2, scorer3),
            scores -> scores.stream().mapToDouble(Double::doubleValue).average().orElse(0.0)
        );
    ...
    void testLevenshteinDistanceCalculation() {
        ...
        for (var testCase : testCases) {
            ...
            var expectedScore = maxLen == 0 ? 1.0 : 1.0 - (double) testCase.expectedDistance / maxLen;
