...
class EvaluationIntegrationTest {
    ...
    void testEvaluationWithTracing() {
        ...
        var results =
                Evaluation.<String, String>builder()
                        ...
                        .scorer(
                                "length",
                                (input, output) -> output.length() == input.length() ? 1.0 : 0.0)
                        .run();
    ...
    void testEvaluationWithExperimentContext() {
        // Given
        var context = BraintrustContext.forExperiment("exp-123").storeInContext(Context.current());
    ...
    void testParallelEvaluationWithMetrics() {
        ...
        // When
        var results =
                Evaluation.<Integer, String>builder()
                        ...
                        .scorer(
                                "format_check",
                                (input, output) -> output.equals("Result-" + input) ? 1.0 : 0.0)
                        .parallel(true)
    ...
    void testEvaluationWithMixedResults() {
        ...
        // When
        var results =
                Evaluation.<TestCase, Integer>builder()
                                ...
                                tc -> {
                                    if (tc.shouldFail) {
                                        throw new RuntimeException(
                                                "Simulated failure for " + tc.number);
                                    }
                                    return tc.number * 10;
                                })
                        .scorer(
                                "validation",
                                (input, output) -> output == input.number * 10 ? 1.0 : 0.0)
                        .run();
    ...
    void testEvaluationResultsAnalysis() {
        ...
        // When
        var results =
                Evaluation.<Map.Entry<Integer, Integer>, Integer>builder()
                        .name("analysis-eval")
                        .data(data)
