...
class EvaluationTest {
    ...
    void testSimpleEvaluation() {
        ...
        var results = Evaluation.<String, String>builder()
            ...
            .scorer("exact_match", (input, output) -> 
                output.equals(input.toUpperCase()) ? 1.0 : 0.0)
            .run();
    ...
    void testEvaluationWithErrors() {
        ...
        var results = Evaluation.<Integer, Integer>builder()
            ...
            .scorer("result_check", (input, output) -> 
                output == 10 / input ? 1.0 : 0.0)
            .run();
        ...
        var errorResult = results.results().stream()
        ...
        assertThat(errorResult.error().get())
    ...
    void testParallelEvaluation() {
        ...
        var totalDuration = results.results().stream()
    ...
    void testMultipleScorerEvaluation() {
        ...
        var results = Evaluation.<TestCase, String>builder()
            ...
            .scorer("exact_match", (tc, output) -> 
                output.equals(tc.expected) ? 1.0 : 0.0)
            .scorer("length_match", (tc, output) -> 
                output.length() == tc.expected.length() ? 1.0 : 0.0)
            .scorer("starts_with", (tc, output) -> 
                output.startsWith(tc.expected.substring(0, 1)) ? 1.0 : 0.0)
            .run();
        
        // Then
        assertThat(results.summary().scoreStatistics()).hasSize(3);
        assertThat(results.summary().scoreStatistics()).containsKeys(
            "exact_match", "length_match", "starts_with"
        );
        
        // All scorers should have perfect scores
        results.summary().scoreStatistics().values().forEach(stats -> {
    ...
    void testEvaluationWithTimeout() {
        ...
        var fastResult = results.results().stream()
        ...
        var slowResult = results.results().stream()
    ...
    void testStreamDataSource() {
        // Given
        var dataStream = IntStream.range(0, 10)
            .mapToObj(i -> "item" + i);
        
        // When
        var results = Evaluation.<String, Integer>builder()
            .name("stream-eval")
            .data(dataStream)
            .task(String::length)
            .scorer("length_check", (input, output) -> 
                output == input.length() ? 1.0 : 0.0)
            .run();
