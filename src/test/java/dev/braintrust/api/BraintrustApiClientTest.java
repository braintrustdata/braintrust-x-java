...
class BraintrustApiClientTest {
    ...
    void testCreateProject() throws Exception {
        ...
        verify(mockHttpClient).sendAsync(argThat(request -> {
            assertThat(request.uri().toString()).isEqualTo("https://api.test.dev/projects");
    ...
    void testCreateExperiment() throws Exception {
        // Given
        var request = new BraintrustApiClient.CreateExperimentRequest(
            "project-123",
            "Test Experiment",
    ...
    void testInsertDatasetEvents() throws Exception {
        ...
        var events = List.of(
            ...
            new BraintrustApiClient.DatasetEvent("input2", Optional.of("output2"), Optional.of("metadata"))
        );
        ...
        verify(mockHttpClient).sendAsync(argThat(request -> {
            assertThat(request.uri().toString())
                .isEqualTo("https://api.test.dev/datasets/dataset-123/insert");
            assertThat(request.method()).isEqualTo("POST");
            return true;
        }), any());
    }
    ...
    void testDebugLogging() throws Exception {
        // Given
        var debugConfig = BraintrustConfig.builder()
            .apiKey("test-api-key")
            .debug(true)
            .build();
