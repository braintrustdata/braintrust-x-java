...
class BraintrustTracingTest {
    ...
    void testQuickstartWithCustomization() {
        ...
        var otel =
                BraintrustTracing.quickstart(
                        config,
                        builder ->
                                builder.serviceName("custom-service")
    ...
    void testGetTracerWithSpecificInstance() {
        // Given
        var otel = BraintrustTracing.quickstart(config, builder -> builder.registerGlobal(false));
    ...
    void testSpanUtilsAddScore() {
        ...
        assertThat(spanData.getAttributes().get(BraintrustSpanProcessor.SCORE_NAME))
                .isEqualTo("accuracy");
        assertThat(spanData.getAttributes().get(BraintrustSpanProcessor.SCORE)).isEqualTo(0.95);
    }
    ...
    void testBuilderResourceAttributes() {
        ...
        var otel =
                BraintrustTracing.quickstart(
                        config,
                        builder ->
                                builder.serviceName("test-service")
                    ...
                    new Thread(
                            () -> {
                                for (int j = 0; j < spansPerThread; j++) {
                                    var span =
                                            tracer.spanBuilder("thread-" + threadId + "-span-" + j)
                                                    .startSpan();
                                    // Simulate some work
                                    try {
                                        Thread.sleep(1);
