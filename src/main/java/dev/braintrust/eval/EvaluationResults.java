...
public record EvaluationResults<INPUT, OUTPUT>(
    ...
    public Map<Boolean, List<EvaluationResult<INPUT, OUTPUT>>> partitionBySuccess() {
        return results.stream()
            .collect(Collectors.partitioningBy(EvaluationResult::isSuccess));
    }
    ...
    public List<EvaluationResult<INPUT, OUTPUT>> withMinimumScore(double threshold) {
        return results.stream()
            .filter(r -> r.averageScore() >= threshold)
            .toList();
    }
    ...
    public String generateReport() {
        ...
        sb.append("  Success Rate: ").append(String.format("%.2f%%", summary.successRate() * 100)).append("\n");
        ...
        if (!summary.scoreStatistics().isEmpty()) {
            ...
            summary.scoreStatistics().forEach((scorer, stats) -> {
                sb.append("  ").append(scorer).append(":\n");
                sb.append("    Mean: ").append(String.format("%.4f", stats.mean())).append("\n");
                sb.append("    Min: ").append(String.format("%.4f", stats.min())).append("\n");
                sb.append("    Max: ").append(String.format("%.4f", stats.max())).append("\n");
                sb.append("    Median: ").append(String.format("%.4f", stats.median())).append("\n");
                sb.append("    Std Dev: ").append(String.format("%.4f", stats.standardDeviation())).append("\n");
            });
        }
