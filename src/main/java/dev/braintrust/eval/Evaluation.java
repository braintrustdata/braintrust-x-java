...
/**
 * Main evaluation framework for testing AI models.
 * Uses Java generics for type safety and streams for functional processing.
 * 
...
public final class Evaluation<INPUT, OUTPUT> {
    ...
    public EvaluationResults<INPUT, OUTPUT> run() {
        var span = tracer.spanBuilder("evaluation")
            .setSpanKind(SpanKind.INTERNAL)
            .setAttribute("evaluation.name", name)
            .startSpan();
        
        try (var scope = span.makeCurrent()) {
            return options.experimentId != null
                ? runWithExperiment(span)
                : runStandalone(span);
        } finally {
            span.end();
        }
    ...
    private EvaluationResult<INPUT, OUTPUT> evaluateOne(INPUT input, Span parentSpan) {
        var span = tracer.spanBuilder("evaluation.task")
    ...
    private Map<String, Double> calculateScores(INPUT input, @Nullable OUTPUT output, @Nullable Exception error) {
    ...
    private EvaluationSummary calculateSummary(List<EvaluationResult<INPUT, OUTPUT>> results) {
        ...
        var avgDuration = results.stream()
    ...
    private Map<String, ScoreStatistics> calculateScoreStatistics(List<EvaluationResult<INPUT, OUTPUT>> results) {
        var scoresByName = new HashMap<String, List<Double>>();
        
        results.stream()
            .flatMap(r -> r.scores().entrySet().stream())
            .forEach(entry -> scoresByName
    ...
    public static final class Builder<INPUT, OUTPUT> {
        ...
        public Builder<INPUT, OUTPUT> scorer(String name, BiFunction<INPUT, OUTPUT, Double> scoreFunc) {
