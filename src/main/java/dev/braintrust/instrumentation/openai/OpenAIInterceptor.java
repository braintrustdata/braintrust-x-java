...
/**
 * Interceptor for OpenAI API calls that creates OTEL spans.
 * Uses functional interfaces to avoid hard dependency on OpenAI SDK.
 */
public class OpenAIInterceptor {
        ...
        try (Scope scope = span.makeCurrent()) {
            ...
            if (responseDetails.usage() != null) {
                ...
                span.setAttribute(BraintrustSpanProcessor.USAGE_COMPLETION_TOKENS, 
                    responseDetails.usage().completionTokens());
                span.setAttribute(BraintrustSpanProcessor.USAGE_TOTAL_TOKENS, 
                    responseDetails.usage().totalTokens());
    ...
    public static <REQ, STREAM, RESP> STREAM traceStreamingCompletion(
        ...
        var span = tracer.spanBuilder("openai." + operation + ".stream")
            .setSpanKind(SpanKind.CLIENT)
            .setAttribute("openai.model", requestDetails.model())
            .setAttribute("openai.max_tokens", requestDetails.maxTokens())
            .setAttribute("openai.temperature", requestDetails.temperature())
        ...
        try (Scope scope = span.makeCurrent()) {
            ...
            return streamWrapper.wrap(stream, usage -> {
                if (usage != null) {
                    span.setAttribute(BraintrustSpanProcessor.USAGE_PROMPT_TOKENS, usage.promptTokens());
                    span.setAttribute(BraintrustSpanProcessor.USAGE_COMPLETION_TOKENS, usage.completionTokens());
                    span.setAttribute(BraintrustSpanProcessor.USAGE_TOTAL_TOKENS, usage.totalTokens());
    ...
    public interface StreamWrapper<STREAM, RESP> {
        STREAM wrap(STREAM stream, java.util.function.Consumer<Usage> onComplete, 
