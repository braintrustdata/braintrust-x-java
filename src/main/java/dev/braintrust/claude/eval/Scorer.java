...
/**
 * Represents a scoring function for evaluation results. Can be implemented as a functional
 * interface or using the factory methods.
 *
...
public interface Scorer<INPUT, OUTPUT> {
    ...
    /** Returns the name of this scorer. Default implementation returns the class name. */
    default String name() {
    ...
    final class StringScorers {
        ...
        /** Exact match scorer. */
        public static <I> Scorer<I, String> exactMatch(
                java.util.function.Function<I, String> expectedFunc) {
            return of(
                    "exact_match",
                    (input, output) -> {
        ...
        public static <I> Scorer<I, String> levenshtein(
                java.util.function.Function<I, String> expectedFunc) {
            return of(
                    "levenshtein",
                    (input, output) -> {
        ...
        public static <I> Scorer<I, String> contains(
                java.util.function.Function<I, String> substringFunc) {
            return of(
                    "contains",
                    (input, output) -> {
        ...
        /** Regex match scorer. */
        public static <I> Scorer<I, String> regex(
                java.util.function.Function<I, java.util.regex.Pattern> patternFunc) {
            return of(
                    "regex_match",
                    (input, output) -> {
    ...
    final class NumericScorers {
        ...
        public static <I> Scorer<I, Number> absoluteError(
                java.util.function.Function<I, Number> expectedFunc, double threshold) {
            return of(
                    "absolute_error",
                    (input, output) -> {
                        if (output == null) return 0.0;
                        ...
                        return error <= threshold
                                ? 1.0
                                : Math.max(0.0, 1.0 - error / Math.abs(expected));
                    });
        }

        /** Relative error scorer. */
        public static <I> Scorer<I, Number> relativeError(
                java.util.function.Function<I, Number> expectedFunc, double maxError) {
            return of(
                    "relative_error",
                    (input, output) -> {
                        if (output == null) return 0.0;
    ...
    static <I, O> Scorer<I, O> combine(
            String name,
            java.util.List<Scorer<I, O>> scorers,
            java.util.function.Function<java.util.List<Double>, Double> combiner) {
        return new Scorer<I, O>() {
            @Override
            public double score(I input, O output) {
                var scores = scorers.stream().map(scorer -> scorer.score(input, output)).toList();
    ...
    static <I, O> Scorer<I, O> weightedAverage(java.util.Map<Scorer<I, O>, Double> scorerWeights) {
        return new Scorer<I, O>() {
            ...
            public double score(I input, O output) {
                var totalWeight =
                        scorerWeights.values().stream().mapToDouble(Double::doubleValue).sum();

                return scorerWeights.entrySet().stream()
                                .mapToDouble(
                                        entry ->
                                                entry.getKey().score(input, output)
                                                        * entry.getValue())
                                .sum()
                        / totalWeight;
            }
