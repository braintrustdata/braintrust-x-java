 ...
 *
 * <p>In Braintrust, traces are organized by project. You can set the project in several ways: 1.
 * Via span attributes (braintrust.parent.project_id) 2. Via BraintrustContext 3. Via environment
 * variable BRAINTRUST_PROJECT_ID
 *
 * <p>If no project is specified, Braintrust will create a default project based on the service
 * name.
 */
public class ProjectExample {
    ...
    public static void main(String[] args) {
        try {
            ...
            var config = BraintrustConfig.builder().apiKey(apiKey).build();
            ...
            var openTelemetry =
                    BraintrustTracing.quickstart(
                            ...
                            builder ->
                                    builder.serviceName(
                                                    "my-ai-application") // This becomes the default
                                            // project name
            ...
            var span1 =
                    tracer.spanBuilder("example.default_project")
            ...
            var span2 =
                    tracer.spanBuilder("example.explicit_project")
                            .setSpanKind(SpanKind.INTERNAL)
                            .startSpan();
            ...
            var context =
                    BraintrustContext.builder().projectId("recommendation-engine").storeInCurrent();

            try (Scope contextScope = context.makeCurrent()) {
                // All spans created within this scope will belong to the recommendation-engine
                // project

                var span3a =
                        tracer.spanBuilder("example.context_span_1")
                                .setSpanKind(SpanKind.INTERNAL)
                                .startSpan();
                ...
                var span3b =
                        tracer.spanBuilder("example.context_span_2")
                                .setSpanKind(SpanKind.INTERNAL)
                                .startSpan();
            ...
            try (Scope expScope = experimentContext.makeCurrent()) {
                var span4 =
                        tracer.spanBuilder("example.experiment_run")
                                .setSpanKind(SpanKind.INTERNAL)
                                .startSpan();
