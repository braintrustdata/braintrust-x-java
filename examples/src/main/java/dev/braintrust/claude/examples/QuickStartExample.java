...
/**
 * Quick start example showing basic Braintrust tracing setup. Demonstrates idiomatic Java usage
 * with try-with-resources and functional style.
 */
public class QuickStartExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var customConfig =
                BraintrustConfig.create(
                        builder ->
                                builder.apiKey("your-api-key")
        ...
        try (var ignored = tracer.spanBuilder("simple-operation").startSpan().makeCurrent()) {
        ...
        try (var ignored = tracer.spanBuilder("error-example").startSpan().makeCurrent()) {
    ...
    private static void processItems(List<String> items, io.opentelemetry.api.trace.Tracer tracer) {
        var span =
                tracer.spanBuilder("process-items")
                        .setAttribute("item.count", items.size())
                        .startSpan();

        try (Scope scope = span.makeCurrent()) {
            // Use Java streams for functional processing
            var results = items.parallelStream().map(item -> processItem(item, tracer)).toList();
    ...
    private static boolean processItem(String item, io.opentelemetry.api.trace.Tracer tracer) {
        var span = tracer.spanBuilder("process-item").setAttribute("item.name", item).startSpan();
