...
import dev.braintrust.claude.eval.EvaluationResult;
import dev.braintrust.claude.eval.EvaluationResults;
import dev.braintrust.claude.eval.Evaluation;
import dev.braintrust.claude.eval.Scorer;
...
/**
 * Example showing dataset management and API usage with modern Java patterns. Demonstrates
 * CompletableFuture composition, Optional handling, and functional transformations.
 */
public class DatasetExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var projectFuture =
                apiClient
                        .createProject("qa-evaluation-demo")
                        .exceptionally(
                                error -> {
        ...
        var context = BraintrustContext.forProject(project.id()).storeInContext(Context.current());
        ...
        try (var scope = context.makeCurrent()) {
            ...
            var experimentFuture =
                    apiClient.createExperiment(
            ...
            var results =
                    Evaluation.<QAExample, ModelOutput>builder()
    ...
    private static CompletableFuture<BraintrustApiClient.Dataset> createOrUpdateDataset(
        ...
        return client.createDataset(
                        new BraintrustApiClient.CreateDatasetRequest(
                                projectId,
                                "qa-test-dataset",
    ...
    private static List<QAExample> loadTestData() {
        return List.of(
                new QAExample(
                        ...
                        "France is a country in Western Europe. Its capital city is known for the"
                                + " Eiffel Tower.",
                        "Paris"),
                new QAExample(
                        "Who wrote Romeo and Juliet?",
                        "William Shakespeare was an English playwright and poet of the Renaissance"
                                + " period.",
                        "William Shakespeare"),
                new QAExample(
    ...
    private static String generateAnswer(QAExample example) {
        ...
        if (random > 0.7) {
            ...
            return example.answer.substring(0, Math.min(10, example.answer.length()))
                    + "..."; // Partial
        } else {
    ...
    private static void analyzeResults(
            EvaluationResults<QAExample, ModelOutput> results) {
        System.out.println("\n=== Evaluation Analysis ===");
        ...
        // Confidence analysis
        var avgConfidence =
                results.successful().mapToDouble(r -> r.output().confidence).average().orElse(0.0);
        ...
        results.successful()
                ...
                .forEach(
                        r ->
                                System.out.printf(
                                        "  Q: %s%n  A: %s (confidence: %.2f)%n",
                                        r.input().question,
                                        r.output().answer,
                                        r.output().confidence));

        // Score correlation analysis
        var scoresByQuestion =
                results.successful()
                        .collect(
