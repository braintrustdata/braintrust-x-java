...
/**
 * Simple example demonstrating Braintrust with OpenTelemetry.
 * This example doesn't require external API keys and shows basic usage.
 */
public class SimpleExample {
    ...
    public static void main(String[] args) {
        try {
            ...
            if (apiKey == null || apiKey.isEmpty()) {
                System.out.println("\nNote: To see traces in Braintrust, set BRAINTRUST_API_KEY environment variable");
            }
    ...
    private static void performSimpleOperation(io.opentelemetry.api.trace.Tracer tracer) {
        var span = tracer.spanBuilder("simple.operation")
    ...
    private static void performNestedOperations(io.opentelemetry.api.trace.Tracer tracer) {
        var rootSpan = tracer.spanBuilder("nested.root")
            .setSpanKind(SpanKind.INTERNAL)
            .startSpan();
        
        try (Scope rootScope = rootSpan.makeCurrent()) {
            // Child operation 1
            var child1Span = tracer.spanBuilder("nested.child1")
                .setSpanKind(SpanKind.INTERNAL)
                .startSpan();
            ...
            // Child operation 2
            var child2Span = tracer.spanBuilder("nested.child2")
                .setSpanKind(SpanKind.INTERNAL)
                .startSpan();
    ...
    private static void performSimulatedLLMCall(io.opentelemetry.api.trace.Tracer tracer) {
        var span = tracer.spanBuilder("llm.chat.completion")
        ...
        try (Scope scope = span.makeCurrent()) {
            ...
            System.out.println("Tokens used: " + totalTokens + " (cost: $" + String.format("%.4f", cost) + ")");
    ...
    private static void demonstrateLogger() {
        ...
        BraintrustLogger.info("Processing complete: items={} duration_ms={} success_rate={}", 
            100, 1234, 0.95);
