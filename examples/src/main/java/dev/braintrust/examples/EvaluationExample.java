...
/**
 * Example demonstrating the evaluation framework with idiomatic Java.
 * Shows use of records, streams, lambdas, and method references.
 */
public class EvaluationExample {
    ...
    public static void main(String[] args) {
        ...
        var testData = IntStream.range(1, 21)
            .mapToObj(i -> new MathProblem(
        ...
        var results = Evaluation.<MathProblem, String>builder()
        ...
        scoreDistribution.entrySet().stream()
            ...
            .forEach(entry -> System.out.printf("  %.1f: %d results%n", 
        ...
        results.results().stream()
            ...
            .forEach(r -> System.out.printf("  Q: %s, Expected: %d, Got: %s%n",
                r.input().question, r.input().expected, r.output()));
        
        // Performance metrics
        var durations = results.results().stream()
    ...
    private static Scorer<MathProblem, String> createNumericScorer() {
        return Scorer.of("numeric_accuracy", (problem, output) -> {
            try {
