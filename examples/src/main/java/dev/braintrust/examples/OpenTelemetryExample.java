 ...
 * 
 * This example shows:
 * - Setting up Braintrust with OpenTelemetry
 * - Making real OpenAI API calls using the official SDK
 * - Wrapping calls with Braintrust instrumentation
 * - Logging results with custom metadata and scores
 */
public class OpenTelemetryExample {
    ...
    private static void performNonStreamingCompletion(OpenAIClient client, io.opentelemetry.api.trace.Tracer tracer) {
        // Create a parent span for the operation
        var parentSpan = tracer.spanBuilder("example.non_streaming_completion")
        ...
        try (Scope scope = parentSpan.makeCurrent()) {
            ...
            var apiSpan = tracer.spanBuilder("openai.chat.completion")
            ...
            try (Scope apiScope = apiSpan.makeCurrent()) {
                ...
                if (response.usage().isPresent()) {
                    ...
                    apiSpan.setAttribute(BraintrustSpanProcessor.USAGE_PROMPT_TOKENS, usage.promptTokens());
                    apiSpan.setAttribute(BraintrustSpanProcessor.USAGE_COMPLETION_TOKENS, usage.completionTokens());
                    apiSpan.setAttribute(BraintrustSpanProcessor.USAGE_TOTAL_TOKENS, usage.totalTokens());
                    
                    var estimatedCost = estimateCost("gpt-4o", usage.promptTokens(), usage.completionTokens());
    ...
    private static void performComplexOperation(OpenAIClient client, io.opentelemetry.api.trace.Tracer tracer) {
        ...
        try (Scope rootScope = rootSpan.makeCurrent()) {
            ...
            var storySpan = tracer.spanBuilder("expand_story")
                .setSpanKind(SpanKind.INTERNAL)
                .startSpan();
            ...
            var titleSpan = tracer.spanBuilder("generate_title")
                .setSpanKind(SpanKind.INTERNAL)
                .startSpan();
            ...
            System.out.println("\nStory Preview: " + story.substring(0, Math.min(200, story.length())) + "...");
    ...
    private static void demonstrateErrorHandling(OpenAIClient client, io.opentelemetry.api.trace.Tracer tracer) {
        var span = tracer.spanBuilder("example.error_handling")
            .setSpanKind(SpanKind.INTERNAL)
        ...
        try (Scope scope = span.makeCurrent()) {
            ...
            try {
                var apiSpan = tracer.spanBuilder("openai.chat.completion.error")
                    .setSpanKind(SpanKind.CLIENT)
                    .startSpan();
            ...
            var validRequest = ChatCompletionCreateParams.builder()
                .model(ChatModel.GPT_4O)
    ...
    private static void demonstrateLogger() {
        ...
        // Log various events
        BraintrustLogger.info("Application started with version={} environment={}", "1.0.0", "development");
        
        BraintrustLogger.info("API performance metrics: score={} endpoint={} latency_ms={}", 0.95, "/api/chat", 250);
        
        BraintrustLogger.info("Model quality score={} model={} task={}", 0.88, "gpt-4o", "summarization");
        ...
        try {
            ...
            BraintrustLogger.error("Simulated error occurred: error_type={} handled={}", e, "simulation", true);
        }
    ...
    private static String generateStoryOutline(OpenAIClient client) {
        var request = ChatCompletionCreateParams.builder()
            ...
            .addUserMessage("Create a brief outline for a short story about AI and humanity. Include 3 main plot points.")
            .maxTokens(150L)
            .temperature(0.8)
            .build();
